...
func main() {
	...
	for page := 1; page <= totalPages; page++ {
		...
		for i := 1; i <= 3; i++ {
			...
			debtCollectionFee := utils.CleanText(doc.Find(fmt.Sprintf(".attr-DebtCollectionFee .cmpr-col.%s span", col)).Text())
			...
			loanFees := models.LoanFees{
				...
				PrepaymentFee:         utils.CleanText(doc.Find(fmt.Sprintf(".attr-PrepaymentFee .cmpr-col.%s span", col)).Text()),
				ExtensionFee:          utils.CleanText(doc.Find(fmt.Sprintf(".attr-ExtensionFee .cmpr-col.%s span", col)).Text()),
				AnnualFee:             utils.CleanText(doc.Find(fmt.Sprintf(".attr-AnnualFee .cmpr-col.%s span", col)).Text()),
				AppraisalFeeInternal:  utils.CleanText(doc.Find(fmt.Sprintf(".attr-SurveyAndAppraisalFeeByInternal .cmpr-col.%s span", col)).Text()),
				AppraisalFeeExternal:  utils.CleanText(doc.Find(fmt.Sprintf(".attr-SurveyAndAppraisalFeeByExternal .cmpr-col.%s span", col)).Text()),
				DebtCollectionFee:     debtCollectionFee,
				CreditCheckFee:        creditCheckFee,
				StatementReIssuingFee: statementReIssuingFee,

				// Extracted numerical values
				DebtCollectionFeeAmount: utils.ExtractFirstInt(debtCollectionFee),
